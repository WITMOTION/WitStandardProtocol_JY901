...
if __name__ == '__main__':
    ...
    device.ADDR = 0x50                                       #设置传感器ID
    if (platform.system().lower() == 'linux'):
        device.serialConfig.portName = "/dev/ttyUSB0"        #设置串口
    else:
        device.serialConfig.portName = "COM82"               #设置串口
    ...
    startRecord()                                            # 开始记录数据
    t = threading.Thread(target=LoopReadThead, args=(device,))  #开启一个线程读取数据
    t.start()
