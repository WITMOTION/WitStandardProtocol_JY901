...
CaliCmd=[255 170 1 7 0]';  %0xFF 0xAA 0x01 0x07 0x00
OutCaliCmd=[255 170 1 0 0]';  %0xFF 0xAA 0x01 0x00 0x00
...
while(1)
    if CalFlag==0
        CalFlag=input('请输入CALI进行校准','s');
        if strcmp(CalFlag,StartCali)==1
            fwrite(s,UnLockCmd(1:5),'uint8');               %解锁   Unlock
            pause(1);
            fwrite(s,CaliCmd(1:5),'uint8');                 %解锁完成后，等待1秒，发送进入磁场校准   After unlocking, wait for 1 second and send command to enter the magnetic field calibration
            disp('校准中，请分别绕3个轴慢速各旋转一圈，60秒后自动退出校准')
            pause(60);
            fwrite(s,UnLockCmd(1:5),'uint8');               %解锁   Unlock
            pause(1);
            fwrite(s,OutCaliCmd(1:5),'uint8');              %等待60秒，校准完成后，退出校准   Wait 60 seconds, after the calibration is complete, exit the calibration
            pause(1);
            fwrite(s,SaveCmd(1:5),'uint8');                 %等待1秒，保存校准参数   Wait 1 second to save the calibration parameters
            disp('校准完成，请Y轴指北，确认是否正确')
            pause(1);
        end
    end
